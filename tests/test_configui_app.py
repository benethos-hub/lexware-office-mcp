...
# -- which files this interface works on ------------------------------------


def test_the_interface_can_be_pointed_at_other_files(
    browser: Browser, installation: Installation, tmp_path: Path, monkeypatch
) -> None:
    from benethos_lexware_office_mcp.configui import pins as pins_module

    pointers = tmp_path / "setup.json"
    monkeypatch.setattr(pins_module, "pins_file", lambda: pointers)
    other_env = tmp_path / "other" / ".env"
    other_policy = tmp_path / "other" / "tools.json"

    _, body, _ = browser.post(
        "/files", {"env_file": str(other_env), "tools_file": str(other_policy)}
    )

    assert installation.env_path == other_env
    assert installation.settings.policy_file() == other_policy
    assert json.loads(pointers.read_text(encoding="utf-8")) == {
        "envFile": str(other_env),
        "toolsFile": str(other_policy),
    }
    assert "bearbeitet jetzt" in note(body)


def test_the_named_policy_survives_a_later_reload(
    browser: Browser, installation: Installation, tmp_path: Path, monkeypatch
) -> None:
    """Saving anything resolves the settings again, which used to lose it."""
    from benethos_lexware_office_mcp.configui import pins as pins_module

    monkeypatch.setattr(pins_module, "pins_file", lambda: tmp_path / "setup.json")
    other_policy = tmp_path / "other" / "tools.json"
    browser.post(
        "/files",
        {"env_file": str(installation.env_path), "tools_file": str(other_policy)},
    )

    browser.post("/settings", {"LXO_MCP_PAGE_SIZE": "60"})

    assert installation.settings.policy_file() == other_policy


def test_a_directory_is_not_a_file(
    browser: Browser, installation: Installation, tmp_path: Path
) -> None:
    _, body, _ = browser.post("/files", {"env_file": str(tmp_path), "tools_file": ""})

    assert "keine Datei" in note(body)
    assert installation.env_path != tmp_path


def test_without_an_env_path_nothing_moves(
    browser: Browser, installation: Installation
) -> None:
    before = installation.env_path

    _, body, _ = browser.post("/files", {"env_file": "  ", "tools_file": ""})

    assert "Ohne .env-Pfad" in note(body)
    assert installation.env_path == before
